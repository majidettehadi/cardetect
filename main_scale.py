...
TEST_DIC = "CarData/TestImages_Scale/"
TEST_SPLIT_DIC = "result/test_split_scale/"
...
if __name__ == "__main__":
    ...
    no_points_number = 0
    with open("result/foundLocations_scale.txt", 'w') as result:
        for i_d in range(len(test_data)):
            ...
            if test_f_name[i_d] in TEST_DAMAGED_FILES:
                ...
                line = line + str(int(len(test_data[i_d]) / 2)) + ",100)"
                result.write(line + "\n")
                continue
            ...
            if found_points is not None:
                for found_point in found_points:
                    ...
                    line = line + str(found_point[0]) + ",100)"
